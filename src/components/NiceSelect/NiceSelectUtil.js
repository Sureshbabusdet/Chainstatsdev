...
function triggerClick(el) {
  ...
  NiceSelect.prototype.renderDropdown = function() {
    ...
    let searchHtml = `<div className="nice-select-search-box">
  <input type="text" className="nice-select-search" placeholder="Search..."/>
  </div>`;
  
    var html = `<div className="${classes.join(' ')}" tabindex="${
      this.disabled ? null : 0
    }">
    <span className="${this.multiple ? 'multiple-options' : 'current'}"></span>
  ...
  NiceSelect.prototype._renderSelectedItems = function() {
    if (this.multiple) {
      ...
      this.selectedOptions.forEach(function(item) {
        selectedHtml += `<span className="current">${item.data.text}</span>`;
      });
